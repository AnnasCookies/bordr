...
> read files from it. There is no login.**
...
```
your phone ──tailnet──> tailscale serve ──127.0.0.1──> bordr ──unix socket──> herdr
```
...
| **CSRF**                     | State-changing `/api/` requests are rejected when `Sec-Fetch-Site` says anything but same-origin, or when the `Origin` host is not the host the request arrived at; either stops a sandboxed artifact driving your agents. Requests with neither header (curl, scripts on the box) pass; the tailnet is their boundary |
| **Hostname**                 | Any `Host` bordr was not told to serve gets `421`, against DNS rebinding (above)                                                                                                                                                                                                                                       |
| **Keypad allowlist**         | The on-screen keys accept only inert navigation keys. No control sequences and no text injection; text goes through the prompt endpoint                                                                                                                                                                                |
| **Transcript escaping**      | Terminal output is HTML-escaped before rendering; the ANSI renderer emits only colour spans                                                                                                                                                                                                                            |
| **Markdown**                 | Rendered with `marked`, then sanitised with DOMPurify with `style`, `form` and `iframe` forbidden                                                                                                                                                                                                                      |
